    ...
    public class SQLConnection
    {
        ...
        public static void TryConnectTODB()
        {
            try
            {
                ...
                cmd.CommandText = ("IF NOT EXISTS (SELECT * FROM sys.tables WHERE [name] = 'swp4_equipInUse') CREATE TABLE swp4_equipInUse ([Id] INT IDENTITY(1, 1) NOT NULL, [equipID] INT NULL, [date] VARCHAR(50) NULL, PRIMARY KEY CLUSTERED([Id] ASC))");
                cmd.ExecuteNonQuery();
                con.Close();
            }
            catch (Exception e)
